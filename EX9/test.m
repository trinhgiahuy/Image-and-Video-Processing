...
try
	...
	originalFolder = pwd; 
	folder = 'C:\Users\Umer\Documents\Temporary'; 
	if ~exist(folder, 'dir') 
		folder = pwd; 
	end 
	cd(folder); 
...
% 	[valueThresholdLow, valueThresholdHigh] = threshold(valueThresholdLow, valueThresholdHigh, vImage);
	...
	% Get the areas of the tenative blobs.
...
try
	...
	maxYValue = ylim;
	maxXValue = xlim;
	line([lowThresh lowThresh], [maxYValue(1) maxYValue(2)], 'Color', 'm', 'LineWidth', 3);
	line([highThresh highThresh], [maxYValue(1) maxYValue(2)], 'Color', 'm', 'LineWidth', 3);

...
class EdgeDetect_Algorithm{
 public:
  ...
  //-----------------------------------------------------------------------------
  // Function: run
  // Top interface for data inout of class. Combines vertical and horizontal derivative and magnitude/angle computation.
	   ...
	   unsigned int imageHeight)
  {
    ...
    double *dy = (double *)malloc(imageHeight*imageWidth*size);
    double *dx = (double *)malloc(imageHeight*imageWidth*size);
			    ...
			    unsigned int imageHeight)
  {
	...
	*(dy + y*imageWidth + x) =
	  dat_in[clip(y - 1, imageWidth-1) * imageWidth + x] * kernel[0] +
	  dat_in[y * imageWidth + x]                         * kernel[1] +
	  dat_in[clip(y + 1, imageWidth-1) * imageWidth + x] * kernel[2];
      }    
    }
  }
  ...
  // Compute the magnitude and angle based on the horizontal and veritcal derivative result
